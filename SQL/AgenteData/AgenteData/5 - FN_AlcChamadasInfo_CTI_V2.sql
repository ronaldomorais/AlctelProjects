...
DECLARE 
	...
	,@data_inicio DATETIME = '2023-02-01 00:00:00'
	,@data_fim DATETIME = '2023-02-03 23:59:59'
	,@site_id INT = 0
	...
	SELECT 
		...
		,cha_outFila
		,evt_number
	FROM AlctelSRVCTI.dbo.Alc_chamadasGeralFilas
	WHERE
	...
	SELECT 
		...
		,evt_abandonado
		,evt_number
	FROM AlctelSRVCTI.dbo.Alc_chamadasGeral
	WHERE
		cha_ini BETWEEN @_data_inicio AND @_data_fim
	...
	SELECT 
		...
		,evt_transferencia
		,evt_number
	FROM AlctelSRVCTI.dbo.Alc_chamadasGeralConversacao
	WHERE
	...
	SELECT 
		...
		,age_fim
		,evt_number
	FROM AlctelSRVCTI.dbo.Alc_chamadasGeralNaoAtende
	WHERE
		age_ini BETWEEN @_data_inicio AND @_data_fim		
	...
	SELECT 
		...
		,cha_origem
		--,cha_fila
		,CASE WHEN cha_evento_nome = '3 - AGENTE_ATENDE' AND cha_fila = ''
			THEN LAG(cha_fila, 1, '') OVER(PARTITION BY cha_id ORDER BY cha_evento_hora)
			ELSE cha_fila
		END cha_fila
		--,cha_inFila
		,CASE WHEN cha_evento_nome = '3 - AGENTE_ATENDE' AND cha_inFila IS NULL
			THEN LAG(cha_inFila, 1, '') OVER(PARTITION BY cha_id ORDER BY cha_evento_hora)
			ELSE cha_inFila
		END cha_inFila
		--,cha_outFila
		,CASE WHEN cha_evento_nome = '3 - AGENTE_ATENDE' AND cha_inFila IS NULL
			THEN LAG(cha_outFila, 1, '') OVER(PARTITION BY cha_id ORDER BY cha_evento_hora)
			ELSE cha_outFila
		END cha_outFila
		,age_agentid
		,age_extensao
