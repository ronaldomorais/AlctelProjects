...
/****** Object:  StoredProcedure [dbo].[TESTE_Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalheFila]    Script Date: 09/11/2022 22:56:28 ******/
SET ANSI_NULLS ON
GO
...
--DECLARE
--	@dataInicial VARCHAR(10) = '2022-11-07'
--	,@dataFinal VARCHAR(10)  = '2022-11-11'
--	,@parSite INT = 11

CREATE PROCEDURE [dbo].[TESTE_Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalheFila] 
...
--PROC Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalhe transcrita aqui por causa de proecdures nested
	...
	IF OBJECT_ID('tempdb..#tDadosDetalhadosUltimaChamadaAntesPesquisa') IS NOT NULL
		DROP TABLE #tDadosDetalhadosUltimaChamadaAntesPesquisa
	...
	SELECT
		...
		,S.AbandonadasAte20
		,S.AbandonadasAte30
	INTO #tDadosDetalhadosUltimaChamadaAntesPesquisa
	FROM (
		SELECT 
			cha_data
			,cha_fila
			...
			,AbandonadasAte20
			,AbandonadasAte30
			,RANK() OVER(PARTITION BY Recebidas ORDER BY cha_data DESC) ordem_desc_atendimento
		FROM #dadosDetalhados	
	) AS S
	WHERE
		S.ordem_desc_atendimento = 1
	...
	WHERE
		evt_connid IN(
			SELECT DISTINCT Atendidas FROM #tDadosDetalhadosUltimaChamadaAntesPesquisa
		)
	...
	SELECT 
		...
		,CASE 
			WHEN R.inicio_pesquisa IS NOT NULL 
				...
				ELSE 'N/A'
			END AS resposta_2	
		,RANK() OVER(PARTITION BY C.Atendidas ORDER BY C.cha_data, protocolo) AS seq
	FROM #tDadosDetalhadosUltimaChamadaAntesPesquisa AS C 
	...
	DELETE FROM #tDadosDasPesquisas WHERE seq > 1
--PROC Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalhe transcrita aqui por causa de proecdures nested
	...
	SELECT 
		...
		,S.atendidas
		,S.enviados_pesquisa
		,S.pesquisas_respondidas
		,S.pergunta1_resp_sim
		,S.pergunta1_resp_sim_perc
		,S.pergunta1_resp_nao
		,S.pergunta1_resp_nao_perc
		--,CASE WHEN (S.pergunta1_resp_sim + S.pergunta1_resp_nao) > 0
		--	THEN ROUND(((CAST(S.pergunta1_resp_sim AS FLOAT) / CAST((S.pergunta1_resp_sim + S.pergunta1_resp_nao) AS FLOAT)) - (CAST(S.pergunta1_resp_nao AS FLOAT) / CAST((S.pergunta1_resp_sim + S.pergunta1_resp_nao) AS FLOAT))) * 100, 2)
		--	ELSE 0
		--END AS resolutividade
		,CASE WHEN (S.pesquisas_respondidas) > 0
			THEN ROUND((CAST(S.pergunta1_resp_sim AS FLOAT) / CAST(S.pesquisas_respondidas AS FLOAT)) * 100, 2)
			ELSE 0
		END AS resolutividade
		,S.pergunta2_resp1
		,S.pergunta2_resp1_perc
		,S.pergunta2_resp2
		,S.pergunta2_resp2_perc
		,S.pergunta2_resp3
		,S.pergunta2_resp3_perc
		,S.pergunta2_resp4
		,S.pergunta2_resp4_perc
		,S.pergunta2_resp5
		,S.pergunta2_resp5_perc
		,S.nota_geral_per
	FROM (	
