...
/****** Object:  StoredProcedure [dbo].[TESTE_Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalheAgente]    Script Date: 21/11/2022 11:05:07 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[TESTE_Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalheAgente] 
...
--PROC Alc_Paae_001_sp_hiPesquisaSatisfacaoDetalhe transcrita aqui por causa de proecdures nested
	...
	SELECT [data]
		...
		,SUM(enviado_pesquisa) AS enviado_pesquisa
		,SUM(respondida_pesquisa) AS responderam_pesquisa
		,CASE 
			WHEN SUM(enviado_pesquisa) > 0
				THEN ROUND(CAST(SUM(respondida_pesquisa) AS FLOAT) / CAST(SUM(enviado_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS responderam_pesquisa_per
		,SUM(resposta_sim) AS respostas_sim
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_sim) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_sim_per
		,SUM(resposta_nao) AS respostas_nao
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nao) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nao_per
		,SUM(resposta_nota5) AS respostas_nota5
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nota5) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nota5_per
		,SUM(resposta_nota4) AS respostas_nota4
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nota4) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nota4_per
		,SUM(resposta_nota3) AS respostas_nota3
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nota3) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nota3_per
		,SUM(resposta_nota2) AS respostas_nota2
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nota2) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nota2_per
		,SUM(resposta_nota1) AS respostas_nota1
		,CASE 
			WHEN SUM(respondida_pesquisa) > 0
				THEN ROUND(CAST(SUM(resposta_nota1) AS FLOAT) / CAST(SUM(respondida_pesquisa) AS FLOAT) * 100, 2)
			ELSE 0
			END AS respostas_nota1_per
		...
		,SUM(resposta_nota4) + SUM(resposta_nota5) AS nota_geral_num
		,SUM(resposta_nota1) + SUM(resposta_nota2) + SUM(resposta_nota3) + SUM(resposta_nota4) + SUM(resposta_nota5) AS nota_geral_den
