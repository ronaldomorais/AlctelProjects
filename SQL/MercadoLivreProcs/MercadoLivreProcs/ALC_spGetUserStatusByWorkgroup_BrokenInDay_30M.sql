...
/****** Object:  StoredProcedure [Alc_rel].[ALC_spGetUserStatusByWorkgroup_BrokenInDay_30M]    Script Date: 18/06/2023 10:56:55 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [Alc_rel].[ALC_spGetUserStatusByWorkgroup_BrokenInDay_30M]
	@dataInicio DATETIME
...
BEGIN	
	...
	--DECLARE @dataInicio DATETIME = '2018-09-12 00:00:00'
	--DECLARE @dataTermino DATETIME = '2018-09-12 23:59:59'	
...
--Quebrando o tempo das chamadas em tempos de 30 em 30 minutos
	...
	IF OBJECT_ID('tempdb..#tbStatusInvervals') IS NOT NULL
		DROP TABLE #tbStatusInvervals
	IF OBJECT_ID('tempdb..#tbStatusIntervalLevel') IS NOT NULL
		DROP TABLE #tbStatusIntervalLevel
	IF OBJECT_ID('tempdb..#tbStatusNotNeedToBreakIn30Min') IS NOT NULL
		DROP TABLE #tbStatusNotNeedToBreakIn30Min
	IF OBJECT_ID('tempdb..#tbStatusToBreakIn30Min') IS NOT NULL
		DROP TABLE #tbStatusToBreakIn30Min
	IF OBJECT_ID('tempdb..#tbStatusBrokenIn30Min') IS NOT NULL
		DROP TABLE #tbStatusBrokenIn30Min
	IF OBJECT_ID('tempdb..#tbUserStatusByWGBrokenInDays') IS NOT NULL
		DROP TABLE #tbUserStatusByWGBrokenInDays
	...
	SELECT 
		...
		,StatusDateTimeOnDay
		--,EndDateTimeOnDay
		,CASE WHEN CAST(StatusDateTimeOnDay AS date) < CAST(EndDateTimeOnDay AS date)
			THEN DATEADD(SECOND, -1, EndDateTimeOnDay)
			ELSE EndDateTimeOnDay
		END AS EndDateTimeOnDay
		,StateDurationOnDay
		,CASE WHEN CAST(StatusDateTimeOnDay AS date) < CAST(EndDateTimeOnDay AS date)
			THEN 1
			ELSE 0
		END AS EndDateTimeOnDayModified
		,I3TimeStampGMT
		,SeqNo	
	INTO #tbUserStatusByWGBrokenInDays
	FROM #tbUserStatusByWorkgroupBrokenInDays
	...
	SELECT
		...
		,EndDateTimeOnDay
		,StateDurationOnDay
		,EndDateTimeOnDayModified
		,I3TimeStampGMT
		,SeqNo	
		,CAST(DATEADD(SECOND, -(DATEDIFF(SECOND, 0, CAST(StatusDateTimeOnDay AS time))%1800), CAST(StatusDateTimeOnDay AS time)) AS time(0)) AS IntervaloInicio
		,CAST(DATEADD(SECOND, -(DATEDIFF(SECOND, 0, CAST(EndDateTimeOnDay AS time))%1800), CAST(EndDateTimeOnDay AS time)) AS time(0)) AS IntervaloTermino
	INTO #tbStatusInvervals
	FROM #tbUserStatusByWGBrokenInDays
	...
	SELECT
		...
		,IntervaloInicio
		,IntervaloTermino
		,((DATEDIFF(SECOND, CAST(CONCAT(CONVERT(VARCHAR(10), StatusDateTimeOnDay, 120), ' ',IntervaloInicio) AS datetime), EndDateTimeOnDay))/1800) AS Nivel
	INTO #tbStatusIntervalLevel
	FROM #tbStatusInvervals

	

	--Separa os status que não precisarão quebrar nos intervalos de 30 minutos.
	SELECT 
		SequenceId
		,UserId_Seq
		,UserId
		...
		,StatusDateTimeOnDay
		--,EndDateTimeOnDay
		,CASE WHEN EndDateTimeOnDayModified = 1 AND CAST(EndDateTimeOnDay AS time(0)) = '23:59:59'
			THEN DATEADD(SECOND, 1, EndDateTimeOnDay)
			ELSE EndDateTimeOnDay
		END AS EndDateTimeOnDay
		,StateDurationOnDay
		,EndDateTimeOnDayModified
		,DATEADD(SECOND, 0, IntervaloInicio) AS Intervalo
		,StatusDateTimeOnDay AS StatusDateTime_30M
		--,EndDateTimeOnDay AS EndDateTime_30M
		,CASE WHEN EndDateTimeOnDayModified = 1 AND CAST(EndDateTimeOnDay AS time(0)) = '23:59:59'
			THEN DATEADD(SECOND, 1, EndDateTimeOnDay)
			ELSE EndDateTimeOnDay
		END AS EndDateTime_30M
		,StateDurationOnDay AS StateDuration_30M
		,I3TimeStampGMT
		,SeqNo	
		,IntervaloInicio
		,IntervaloTermino
		,Nivel
	INTO #tbStatusNotNeedToBreakIn30Min
	FROM #tbStatusIntervalLevel
	WHERE Nivel = 0

	

	--Separa os status que precisarão quebrar nos intervalos de 30 minutos.
	SELECT 
		SequenceId
		,UserId_Seq
		,UserId
	...
	;WITH cte --CTE2
		(
			SequenceId,UserId_Seq,UserId,LoginDateTime,LogoutDateTime,LoginDuration,Workgroup,LoginDateTimeInWorkgroup
			,LogoutDateTimeInWorkgroup,LoginDurationInWorkgroup,StatusKey,StatusDateTime,EndDateTime,StateDuration,StatusDateTimeOnDay
			,EndDateTimeOnDay,StateDurationOnDay,EndDateTimeOnDayModified, Intervalo, StatusDateTime_30M, EndDateTime_30M,I3TimeStampGMT,SeqNo,IntervaloInicio
			,IntervaloTermino,Nivel
		)
	AS
	(
		SELECT
			...
			,DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, 1800*Nivel, IntervaloInicio)), CAST(CAST(StatusDateTimeOnDay AS date) AS datetime)) AS StatusDateTime_30M
			,CAST(EndDateTimeOnDay AS datetime) AS EndDateTime_30M
			,I3TimeStampGMT
			,SeqNo	
			,IntervaloInicio
			,IntervaloTermino
			,Nivel
		FROM #tbStatusToBreakIn30Min
		...
		SELECT
			...
			,S.EndDateTime
			,S.StateDuration
			,S.StatusDateTimeOnDay
			,S.EndDateTimeOnDay
			,S.StateDurationOnDay
			,S.EndDateTimeOnDayModified
			,DATEADD(SECOND, (1800*(C.Nivel-1)), S.IntervaloInicio) AS Intervalo
			,CASE WHEN (C.Nivel - 1) = 0
				THEN CAST(S.StatusDateTimeOnDay AS datetime)
				ELSE DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, (1800*(C.Nivel-1)), S.IntervaloInicio)), CAST(CAST(S.StatusDateTimeOnDay AS date) AS datetime)) 
			END AS StatusDateTime_30M
			,DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, 1800*C.Nivel, S.IntervaloInicio)), CAST(CAST(S.EndDateTimeOnDay AS date) AS datetime)) AS EndDateTime_30M
			,S.I3TimeStampGMT
			,S.SeqNo	
			,S.IntervaloInicio
			,S.IntervaloTermino
			,(C.Nivel-1) AS Nivel
		FROM #tbStatusToBreakIn30Min AS S
			--INNER JOIN cte AS C ON C.UserId = S.UserId AND C.StatusKey = S.StatusKey AND C.StatusDateTimeOnDay = S.StatusDateTimeOnDay
			--	AND C.Workgroup = S.Workgroup
			...
			AND C.EndDateTime = S.EndDateTime
			AND C.StateDuration = S.StateDuration
			AND C.StatusDateTimeOnDay = S.StatusDateTimeOnDay
			AND C.EndDateTimeOnDay = S.EndDateTimeOnDay
			AND C.StateDurationOnDay = S.StateDurationOnDay
			AND C.EndDateTimeOnDayModified = S.EndDateTimeOnDayModified
			AND C.SeqNo = S.SeqNo
			AND C.IntervaloInicio = S.IntervaloInicio
			AND C.IntervaloTermino = S.IntervaloTermino
			--AND C.Nivel = S.Nivel
	...
	SELECT 
		...
		,StatusDateTime_30M
		--,EndDateTime_30M
		,CASE WHEN EndDateTimeOnDayModified = 1 AND CAST(EndDateTime_30M AS time(0)) = '23:59:59'
			THEN DATEADD(SECOND, 1, EndDateTime_30M)
			ELSE EndDateTime_30M
		END AS EndDateTime_30M
		,CASE WHEN EndDateTimeOnDayModified = 1 AND CAST(EndDateTime_30M AS time(0)) = '23:59:59'
			THEN DATEDIFF(SECOND, StatusDateTime_30M, DATEADD(SECOND, 1, EndDateTime_30M)) 
			ELSE DATEDIFF(SECOND, StatusDateTime_30M, EndDateTime_30M) 
		END AS StateDuration_30M
		,I3TimeStampGMT
		,SeqNo	
		,IntervaloInicio
		,IntervaloTermino
		,Nivel
	INTO #tbStatusBrokenIn30Min
	FROM cte
	ORDER BY UserId, StatusDateTimeOnDay, Intervalo
	...
	INSERT INTO #tbStatusBrokenIn30Min
	SELECT 
		SequenceId
		,UserId_Seq
		,UserId
		...
		,IntervaloTermino
		,Nivel
	FROM #tbStatusNotNeedToBreakIn30Min
	...
	BEGIN
		SELECT
			ROW_NUMBER() OVER(ORDER BY UserId, StatusDateTimeOnDay, Intervalo) AS SequenceId
			--SequenceId
			--,UserId_Seq
			,UserId
			--,LoginDateTime
			...
			--,IntervaloTermino
			--,Nivel
		FROM #tbStatusBrokenIn30Min
		WHERE StateDuration_30M > 0
	...
	BEGIN
		SELECT
			...
			--,Nivel
		FROM #tbStatusBrokenIn30Min
		WHERE StateDuration_30M > 0 AND Workgroup = @workgroup
		AND (UserID NOT IN (SELECT [Data] FROM [dbo].[FN_Split] (@ExcludeLoginID, ',')) OR UserID IS NULL)
		ORDER BY UserId, StatusDateTimeOnDay, Intervalo
	END
