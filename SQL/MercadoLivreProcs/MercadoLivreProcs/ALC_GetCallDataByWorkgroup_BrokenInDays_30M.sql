...
/****** Object:  StoredProcedure [Alc_rel].[ALC_GetCallDataByWorkgroup_BrokenInDays_30M]    Script Date: 18/06/2023 10:55:09 ******/
SET ANSI_NULLS ON
GO
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [Alc_rel].[ALC_GetCallDataByWorkgroup_BrokenInDays_30M]
...
--Restante de cálculos, acerto de tempos, etc
	SELECT
		...
		--,CASE WHEN LAG(HorarioTransferencia, 1, NULL) OVER(PARTITION BY CallId ORDER BY dEventTime, SeqNo) IS NULL
		--	THEN 0
		--	ELSE CASE WHEN HorarioEntradaFila IS NULL
		--			THEN 1
		--			ELSE 0
		--		END
		--END AS DeTransferenciaDireta
...
-- Quebrando as chamadas em dias
	...
	--insert into #tbCallsDaysLevel
	--(SeqCallId,	CallId,	Workgroup,	UserId,	FullName,InicioConversacao,TerminoConversacao,TempoConversacao,TempoHold,Nivel)
	--VALUES(1, '100132582860180901','ATENTO','ext_mpivager','Mayara Piva Geraldi','2018-08-01 23:59:50.000','2018-08-02 00:00:10.000',20,10,1)
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Quebrando o tempo das chamadas em tempos de 30 em 30 minutos
	IF OBJECT_ID('tempdb..#tbCallsIntervals') IS NOT NULL
		DROP TABLE #tbCallsIntervals

	IF OBJECT_ID('tempdb..#tbCallsIntervalLevel') IS NOT NULL
		DROP TABLE #tbCallsIntervalLevel
	
	IF OBJECT_ID('tempdb..#tbCallsDataResult') IS NOT NULL
		DROP TABLE #tbCallsDataResult

	IF OBJECT_ID('tempdb..#tbCallsToBreakIn30Min') IS NOT NULL
		DROP TABLE #tbCallsToBreakIn30Min

	IF OBJECT_ID('tempdb..#tbCallsBrokenIn30Min') IS NOT NULL
		DROP TABLE #tbCallsBrokenIn30Min

	IF OBJECT_ID('tempdb..#tbCallDataByWGBrokenInDays') IS NOT NULL
		DROP TABLE #tbCallDataByWGBrokenInDays
	...
	SELECT
		...
		,InicioConversacaoOnDay
		--,TerminoConversacaoOnDay
		,CASE WHEN CAST(InicioConversacaoOnDay AS date) < CAST(TerminoConversacaoOnDay AS date)
			THEN DATEADD(SECOND, -1, TerminoConversacaoOnDay)
			ELSE TerminoConversacaoOnDay
		END AS TerminoConversacaoOnDay
		,TempoConversacaoOnDay
		,TempoHold
		,IsCallBrokenInDays
		,CASE WHEN CAST(InicioConversacaoOnDay AS date) < CAST(TerminoConversacaoOnDay AS date)
			THEN 1
			ELSE 0
		END AS TerminoConversacaoOnDayModified
	INTO #tbCallDataByWGBrokenInDays
	FROM #tbCallDataByWorkgroupBrokenInDays
	--WHERE CallId = '3001179313D0181002'


	--Gerando o intervalo das chamadas
	SELECT 
		SeqCallId
		,CallId
		,Workgroup
		...
		,TempoHold
		,IsCallBrokenInDays
		,TerminoConversacaoOnDayModified
		,CAST(DATEADD(SECOND, -(DATEDIFF(SECOND, 0, CAST(InicioConversacaoOnDay AS time))%1800), CAST(InicioConversacaoOnDay AS time)) AS time(0)) AS IntervaloInicio
		,CAST(DATEADD(SECOND, -(DATEDIFF(SECOND, 0, CAST(TerminoConversacaoOnDay AS time))%1800), CAST(TerminoConversacaoOnDay AS time)) AS time(0)) AS IntervaloTermino
	INTO #tbCallsIntervals
	FROM #tbCallDataByWGBrokenInDays

	--Gerando o nível de quantos intervalos uma chamada ocupa. 
	SELECT
		SeqCallId
		,CallId
		,Workgroup
		...
		,IntervaloInicio
		,IntervaloTermino
		,((DATEDIFF(SECOND, CAST(CONCAT(CONVERT(VARCHAR(10), InicioConversacaoOnDay, 120), ' ',IntervaloInicio) AS datetime), TerminoConversacaoOnDay))/1800) AS Nivel
	INTO #tbCallsIntervalLevel
	FROM #tbCallsIntervals

	--Separa as chamadas que não precisarão quebrar nos intervalos de 30 minutos.
	SELECT 
		SeqCallId
		,CallId
		,Workgroup
		...
		,IntervaloInicio
		,IntervaloTermino
		,DATEADD(SECOND, 1800*Nivel, IntervaloInicio) AS Intervalo
		,InicioConversacaoOnDay AS InicioConversacao_30M
		,TerminoConversacaoOnDay AS TerminoConversacao_30M
		,TempoConversacaoOnDay AS TempoConversacao_30M
		,TempoHold AS TempoHold_30M
		,Nivel
		--,0 AS IsCallBrokenIn30Min
		,IsCallBrokenInDays AS IsCallBrokenIn30Min
	INTO #tbCallsDataResult
	FROM #tbCallsIntervalLevel
	WHERE Nivel = 0

	--Separa as chamadas que precisarão quebrar nos intervalos de 30 minutos.
	SELECT 
		SeqCallId
		,CallId
		,Workgroup
		...
		,IntervaloTermino
		,Nivel
		,1 AS IsCallBrokenIn30Min
	INTO #tbCallsToBreakIn30Min
	FROM #tbCallsIntervalLevel
	WHERE Nivel > 0

	;WITH cte
		(
			SeqCallId,CallId,Workgroup,UserId,FullName,InicioConversacao,TerminoConversacao,TempoConversacao,InicioConversacaoOnDay,TerminoConversacaoOnDay
			,TempoConversacaoOnDay,TempoHold, IsCallBrokenInDays, TerminoConversacaoOnDayModified,IntervaloInicio,IntervaloTermino,Intervalo,InicioConversacao_30M,TerminoConversacao_30M,Nivel,IsCallBrokenIn30Min
		)
	AS
		(
			SELECT
				...
				,IntervaloTermino
				,DATEADD(SECOND, 1800*Nivel, IntervaloInicio) AS Intervalo
				,DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, 1800*Nivel, IntervaloInicio)), CAST(CAST(InicioConversacaoOnDay AS date) AS datetime)) AS InicioConversacao_30M
				,CAST(TerminoConversacaoOnDay AS datetime) AS TerminoConversacao_30M
				,Nivel
				,IsCallBrokenIn30Min
			FROM #tbCallsToBreakIn30Min
			...
			SELECT
				...
				,B.FullName
				,B.InicioConversacao
				,B.TerminoConversacao
				,B.TempoConversacao
				,B.InicioConversacaoOnDay
				,B.TerminoConversacaoOnDay
				,B.TempoConversacaoOnDay
				,B.TempoHold
				,B.IsCallBrokenInDays
				,B.TerminoConversacaoOnDayModified
				,B.IntervaloInicio
				,B.IntervaloTermino
				,DATEADD(SECOND, (1800*(C.Nivel-1)), B.IntervaloInicio) AS Intervalo
				,CASE WHEN (C.Nivel - 1) = 0
					THEN CAST(B.InicioConversacaoOnDay AS datetime)
					ELSE DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, (1800*(C.Nivel-1)), B.IntervaloInicio)), CAST(CAST(B.InicioConversacaoOnDay AS date) AS datetime)) 
				END AS InicioConversacao_30M
				,DATEADD(SECOND, DATEDIFF(SECOND, 0, DATEADD(SECOND, 1800*C.Nivel, B.IntervaloInicio)), CAST(CAST(B.TerminoConversacaoOnDay AS date) AS datetime)) AS TerminoConversacao_30M
				,(C.Nivel-1) AS Nivel
				,CASE WHEN C.Nivel = 1 
					THEN 0 
					ELSE 1
				END AS IsCallBrokenIn30Min
			FROM #tbCallsToBreakIn30Min AS B
				INNER JOIN cte AS C ON C.CallId = B.CallId AND C.SeqCallId = B.SeqCallId AND C.UserId = B.UserId AND C.Workgroup = B.Workgroup AND C.IsCallBrokenInDays = B.IsCallBrokenInDays
			WHERE C.Nivel > 0
		)
		SELECT 
			SeqCallId
			...
			,InicioConversacaoOnDay
			--,TerminoConversacaoOnDay
			,CASE WHEN TerminoConversacaoOnDayModified = 1 AND CAST(TerminoConversacaoOnDay AS time(0)) = '23:59:59'
				THEN DATEADD(SECOND, 1, TerminoConversacaoOnDay)
				ELSE TerminoConversacaoOnDay
			END AS TerminoConversacaoOnDay
			,TempoConversacaoOnDay
			,TempoHold
			,IsCallBrokenInDays
			,0 AS TerminoConversacaoOnDayModified
			,IntervaloInicio
			,IntervaloTermino
			,Intervalo
			,InicioConversacao_30M
			--,TerminoConversacao_30M
			,CASE WHEN InicioConversacao_30M = 1 AND CAST(TerminoConversacao_30M AS time(0)) = '23:59:59'
				THEN DATEADD(SECOND, 1, TerminoConversacao_30M)
				ELSE TerminoConversacao_30M
			END AS TerminoConversacao_30M
			--,DATEDIFF(SECOND, InicioConversacao_30M, TerminoConversacao_30M) AS TempoConversacao_30M
			,CASE WHEN TerminoConversacaoOnDayModified = 1 AND CAST(TerminoConversacao_30M AS time(0)) = '23:59:59'
				THEN DATEDIFF(SECOND, InicioConversacao_30M, DATEADD(SECOND, 1, TerminoConversacao_30M))
				ELSE DATEDIFF(SECOND, InicioConversacao_30M, TerminoConversacao_30M) 
			END AS TempoConversacao_30M
			,Nivel
			--,IsCallBrokenIn30Min
			,CASE WHEN Nivel > 0
				THEN 1
				ELSE 0
			END AS IsCallBrokenIn30Min
		INTO #tbCallsBrokenIn30Min
		FROM cte
		ORDER BY CallId, SeqCallId, InicioConversacaoOnDay, Intervalo
		OPTION (MAXRECURSION 0);
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
---- Associando o tempo de hold para as chamadas quebradas no dia
	IF OBJECT_ID('tempdb..#tbCallsBrokenIn30M_HoldTime') IS NOT NULL
		DROP TABLE #tbCallsBrokenIn30M_HoldTime

	--Separa as chamadas que não precisaram subtrair hold e já uni com as chamadas que não precisavam ser quebradas em 30 minutos.
	INSERT INTO #tbCallsDataResult
	SELECT
		SeqCallId
		,CallId
		,Workgroup
		...
		,InicioConversacao_30M
		,TerminoConversacao_30M
		,TempoConversacao_30M
		,TempoHold AS TempoHold_30M
		,Nivel
		,IsCallBrokenIn30Min
	FROM #tbCallsBrokenIn30Min
	WHERE TempoHold = 0

	-- Separa as chamadas que contém hold e precisam ser subtraído do tempo de conversação.
	SELECT
		SeqCallId
		,CallId
		,Workgroup
		...
		,Nivel
		,IsCallBrokenIn30Min
		,CASE WHEN LAG(TempoHold,1,NULL) OVER(PARTITION BY CallId ORDER BY InicioConversacaoOnDay, Intervalo) IS NULL
			THEN CASE WHEN (TempoConversacao_30M >= TempoHold) THEN 1 ELSE 0 END
			ELSE 
				CASE WHEN LAG(TempoConversacao_30M,1,NULL) OVER(PARTITION BY CallId ORDER BY InicioConversacaoOnDay, Intervalo) >=
					LAG(TempoHold,1,NULL) OVER(PARTITION BY CallId ORDER BY InicioConversacaoOnDay, Intervalo)
					THEN 0 
					ELSE CASE WHEN (TempoConversacao_30M >= TempoHold) THEN 1 ELSE 0 END
				END
		END AS UsarTempoHold
	INTO #tbCallsBrokenIn30M_HoldTime
	FROM #tbCallsBrokenIn30Min
	WHERE TempoHold > 0

	IF OBJECT_ID('tempdb..#tbCallIdithHoldNotProcessed') IS NOT NULL
		DROP TABLE #tbCallIdithHoldNotProcessed

	SELECT CallId INTO #tbCallIdithHoldNotProcessed FROM #tbCallsBrokenIn30M_HoldTime GROUP BY CallId HAVING SUM(UsarTempoHold) = 0
	...
	SELECT
		...
		,InicioConversacao_30M
		,TerminoConversacao_30M
		,CASE WHEN UsarTempoHold = 1 THEN (TempoConversacao_30M - TempoHold) ELSE TempoConversacao_30M END AS TempoConversacao_30M
		,CASE WHEN UsarTempoHold = 1 THEN TempoHold ELSE 0 END AS TempoHold_30M
		--,UsarTempoHold
		,Nivel
		,IsCallBrokenIn30Min
	FROM #tbCallsBrokenIn30M_HoldTime
	WHERE CallId NOT IN(SELECT CallId FROM #tbCallIdithHoldNotProcessed)


	IF OBJECT_ID('tempdb..#tbCallsWithHoldNotProcessed') IS NOT NULL
		DROP TABLE #tbCallsWithHoldNotProcessed
	...
	SELECT
		...
		,Nivel
		,IsCallBrokenIn30Min
	INTO #tbCallsWithHoldNotProcessed
	FROM #tbCallsBrokenIn30M_HoldTime
	WHERE CallId IN(SELECT CallId FROM #tbCallIdithHoldNotProcessed)

	IF OBJECT_ID('tempdb..#tbCallsWithHoldToUpdate') IS NOT NULL
		DROP TABLE #tbCallsWithHoldToUpdate

	SELECT * INTO #tbCallsWithHoldToUpdate FROM #tbCallsWithHoldNotProcessed

	DECLARE @nLines int = (SELECT COUNT(1) FROM #tbCallsWithHoldNotProcessed)
	DECLARE @TempoConversacao_30M int
	DECLARE @tHold int
	DECLARE @seqCallId int
	DECLARE @nivel int
	DECLARE @TempoConversacao_30M_UPDATE int
	DECLARE @TempoHold_UPDATE int
	...
	BEGIN
		SET @callId = ''
		SET @seqCallId = 0
		SET @TempoConversacao_30M = 0
		SET @tHold = 0
		SET @nivel = 0
		SET @TempoConversacao_30M_UPDATE = 0
		SET @TempoHold_UPDATE = 0


		SELECT TOP 1
			@CallId = CallId
			,@seqCallId = SeqCallId
			,@TempoConversacao_30M = TempoConversacao_30M
			,@tHold = TempoHold 
			,@nivel = Nivel
		FROM #tbCallsWithHoldNotProcessed 
		ORDER BY CallId, SeqCallId, InicioConversacaoOnDay, Intervalo

		IF @cId <> @CallId
		BEGIN
			SET @TempoHoldResto = @tHold
			SET @cId = @CallId
		END

		IF @TempoHoldResto >= @TempoConversacao_30M
		BEGIN
			SET @TempoConversacao_30M_UPDATE = 0
			SET @TempoHold_UPDATE = @TempoHoldResto - (@TempoHoldResto - @TempoConversacao_30M)
		END
		ELSE
		BEGIN
			SET @TempoConversacao_30M_UPDATE = @TempoConversacao_30M - @TempoHoldResto
			SET @TempoHold_UPDATE = @TempoHoldResto
		END

		SET @TempoHoldResto = @TempoHoldResto - @TempoConversacao_30M
		IF @TempoHoldResto < 0
			SET @TempoHoldResto = 0

		UPDATE #tbCallsWithHoldToUpdate 
			SET TempoConversacao_30M = @TempoConversacao_30M_UPDATE
				,TempoHold_30M = @TempoHold_UPDATE
		WHERE CallId = @callId AND SeqCallId = @seqCallId AND Nivel = @nivel

		DELETE FROM #tbCallsWithHoldNotProcessed WHERE CallId = @callId AND SeqCallId = @seqCallId AND Nivel = @nivel
		SET @nLines = (SELECT COUNT(1) FROM #tbCallsWithHoldNotProcessed)
	END
	...
	SELECT 
		...
		,Nivel
		,IsCallBrokenIn30Min
	FROM #tbCallsWithHoldToUpdate 


	IF OBJECT_ID('tempdb..#tbCallsResult') IS NOT NULL
		DROP TABLE #tbCallsResult
	...
	SELECT
		...
		,Nivel
		,IsCallBrokenIn30Min
		--,CASE WHEN LAG(CallId,1,NULL) OVER(PARTITION BY CallId ORDER BY InicioConversacaoOnDay, Intervalo) IS NULL
		--	THEN 0 				
		--	ELSE 
		--		CASE WHEN LAG(Workgroup,1,NULL) OVER(PARTITION BY CallId ORDER BY InicioConversacaoOnDay, Intervalo) = Workgroup
		--			THEN 1
		--			ELSE 0
		--		END				
		--END AS IsCallBrokenIn30Min
	INTO #tbCallsResult		
	FROM #tbCallsDataResult
	ORDER BY CallId, SeqCallId, InicioConversacaoOnDay, Intervalo
