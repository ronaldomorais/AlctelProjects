...
ALTER FUNCTION ufn_R_123_STAT_RES (
...
BEGIN
	...
	SELECT 
		CONVERT(DATETIME, R.TIME_KEY, 103) AS [data]
		,A.agente_nome
		,A.agente_dbid
		,A.agente_id
		,A.agente_cod
	...
	FROM DB_GENESYS_DM..R_123_STAT_RES AS R WITH (NOLOCK)
		LEFT JOIN AlctelVSS.dbo.ufn_Agentes() AS A ON R.[OBJECT_ID] = A.agente_cod
	WHERE
